...
function init() {
  ...
  // gui
  const gui = new GUI();
  gui.add(params, 'brightness', 0, 3).step(0.01).onChange(function (value) {
    mainLight.intensity = value;
  });
  gui.addColor(params, 'color').onChange(function (value) {
    textureBack.material.uniforms['color'].value.set(value);
    textureFront.material.uniforms['color'].value.set(value);
    textureRight.material.uniforms['color'].value.set(value);
    textureLeft.material.uniforms['color'].value.set(value);
  });
  gui.open();
